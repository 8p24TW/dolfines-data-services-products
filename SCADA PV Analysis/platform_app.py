...
# ─────────────────────────────────────────────────────────────────────────────
# VIEW: COMPREHENSIVE REPORT INFO
# ─────────────────────────────────────────────────────────────────────────────
...
def _view_comp_info():
    ...
    st.info("""
    The **Comprehensive Analysis Report** is generated by running the full PVPAT pipeline
    (`run_jinja_report.py`) on the complete multi-year SCADA dataset.

    **Steps:**
    1. Ensure all inverter and irradiance CSV files are in the `00orig/` data folder.
    2. Open a terminal in the `SCADA PV Analysis` directory.
    3. Run:  `python run_jinja_report.py`
    4. The PDF will be saved to `_report_test_output/`.

    *In a future platform version, this will be available directly here with a one-click button.*
    """)

    data_dir = site.get("data_dir", "")
    if data_dir:
        st.markdown(f"""
        <div class="sub-hdr">Configured Data Path</div>
        <code style="color:rgba(255,255,255,0.80);font-size:0.85rem;">{data_dir}</code>
        """, unsafe_allow_html=True)
