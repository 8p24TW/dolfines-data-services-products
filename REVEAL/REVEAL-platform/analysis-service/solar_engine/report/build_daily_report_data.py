...
def build_daily_report(
    ...
    out_dir = (Path(out_dir) if out_dir else
               (_ROOT / "_report_test_output" if (_ROOT / "_report_test_output").exists()
                else Path(_tmpmod.gettempdir()) / "repat_reports"))
    out_dir.mkdir(parents=True, exist_ok=True)

    date_str  = report_date.strftime("%Y%m%d")
    site_safe = "".join(c if c.isalnum() else "_" for c in site_cfg["display_name"])
    html_path = out_dir / f"REPAT_Daily_{site_safe}_{date_str}.html"
    pdf_path  = out_dir / f"REPAT_Daily_{site_safe}_{date_str}.pdf"
...
"""
  ...
  <span>REPAT — Daily Performance Report · {site_name}</span>
...
<head>
  ...
  <title>REPAT Daily Report — {site_name} — {date_str}</title>
  <style>{css}</style>
</head>
<body>
{p1}
...
def _fpdf2_pdf(
    ...
    *,
    ...
    def draw_footer(n: int, total: int = 6):
        ...
        pdf.cell(63, 5, f"REPAT -- Daily Performance Report - {site_name}", align="L")
